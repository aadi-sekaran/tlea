// All content for the book. Written for one reader.

export const FOREWORD = {
  title: 'For you',
  body: [
    `There have been millions of apologies from my side, for millions of reasons, and you increasingly told me that you felt they don't mean anything anymore. But this is my final one, the last one ever, to the love of my life. And I thought I should make it beautiful, like her. Something she actually feels means something. Because she means the world to me, and she will for the rest of my life. Love you, my kannamma.`,
    `I never thought of a life without you, who would ask me 'thadavi thariya', who would ask me to get their ice cream, who would I stay up with till late even though I know I am going to be late for work tomorrow and still complaining but still staying up late, who would ask me to make them something and be sooo freaking excited when I do, who will I be excited to come home to, who will I be happy to see every single day no matter what, who will yap about everything they want to only to me, who will I konjify to make them feel precious, who would I wake up and see in the middle of the night and feel at peace, who will ask me to just put a leg on them and do my work while they do theirs, who will I cuddle in the night, all of the above, no one. But I guess it starts tomorrow, and it's pretty scary to even know there's going to be a life for me without you in it. But it's all on me that this is ending. And I am going to be forever, truly grateful for your presence in my life. Your presence will be there throughout my life, know that.`
  ],
  signoff: '— your Ammu'
};

export const APOLOGY = {
  title: 'The apology',
  body: [
    "I am sorry for making you go through so many things. For every night you cried because of me, for every morning you woke up wondering why. For the small daily disappointments that added up to something bigger than either of us could carry. For letting my own noise drown out the sound of you.",
    "I fell from the high pedestal you had put me on. I was foolish enough to lose a diamond, and I want you to know I know it. There is no version of this where I do not know what I had.",
    "I want the best, and only the best, for you. Whatever that looks like, wherever it takes you, whoever it is with. If it is me, I will earn it. If it is not, I will remember I was loved by you, and try to be worthy of that memory."
  ],
  signoff: 'Your Ammu',
  ps: "PS. If you ever want this taken down, tell me, and it comes down that day. This is yours."
};

// Its own page now, between Chapter VII and the farewell letter.
export const LAST_APOLOGY = {
  title: 'The last ever apology, truly',
  eyebrow: 'the last apology',
  paragraphs: [
    `I am sorry for making you go through so many things you never thought would come from me. I fell from the high pedestal you kept me on, and I think that says everything about how far I've fallen. You had kept me somewhere near the sky. Funnily, I am not even on the ground now. I'm down under.`,
    `I am sorry for everything I have done. I am going to be sorry for the rest of my life for being foolish enough to lose a diamond I had in my hand.`,
    `I know now what I would do and how I would be, if I got to do it all over again. But life doesn't work that way. And if it did, if it came around again, you would have to be with me again, and that sucks. I definitely don't like saying this. In fact, I hate it. But I know your life with someone else is going to be as beautiful as you are. You are soon going to be very happy. You are going to have a peaceful life with all the love in the world I was not able to give you the way you wanted.`,
    `I want the best, and only the best, for you.`
  ],
  signoff: `Signing off as tears roll from my eyes for losing the beauty, and emptying my life.\n\nForever yours, and only yours,\nYour Ammu`,
  ps: `PS. This site will be here for us as long as you want. If you want it taken down, just let me know.`
};

// Goes between the last apology and the Final Song.
export const FAREWELL = {
  title: 'So, this is more like a farewell letter to you.',
  eyebrow: 'A farewell letter',
  body: [
    `I started narrating this letter the day you came back from Turkey. I was so excited because I was making something so beautiful for you. I have been working on this for a couple of months now, spending at least an hour in my day or whenever I'm getting the time. The two months I'm saying is on the 25th of August, and I really believe that by the time I finish this entire project, it will be three months.`,
    `This has been such a great experience to look back, get emotional, go through all the photos, all the chats, and everything more about us, traveling to Iceland, Italy and all our times together. I've gone through our chats, I've gone through our photos and I've gone through every single thing which we had together. And looking back at all of it, I think I've started loving you more. I understood you better, what all your needs are, and everything about us and what we've gone through in these couple of months.`,
    `It was such a pleasure to know you in person. And what I have known through these two months is that I've done everything which you might have always wanted me to do, including doing things consistently. Small things, just small things. It was just one hour of a day, spending it on this.`,
    `And through all of this, I have understood what you want from me. I might not understand 100% of everything you want from me, but I understand what you actually want, and why you want them.`,
    `If I had to describe it, I would say that I have taken on something incredibly demanding, but also deeply meaningful to me. I put so much thought, effort, and love into it. Even on the days when I could not give it 100%, I still found myself coming back to it, thinking about what I wanted to write about us. I shared these memories without always knowing exactly why I was doing it, but somehow, it felt necessary.`,
    `So, I have learned a lot, and learned what our relationship is all about in these two months, more than any other time.`,
    `Had you asked me a couple of months ago, I would have honestly said that I believed we were endgame. I truly thought we would build our future together.`,
    `And I wanted this letter to be narrated in English because I think that caches the raw emotions very well and then I have transcribed it. I just wanted this to be somewhat a professing of love, something you would look and feel proud about, that you have this person who thinks about you, this person who loves you, this person who adores you, adorns you, dotes you, every single word which you could use for loving another person.`,
    `Yet I made mistakes many times, even though I never meant to hurt you. I swear, hurting you was never my intention. a 100% of the times.`,
    `You really were the love of my life. You are the love of my life, and you will be the love of my life. There is not a soul in this world who can change that.`,
    `And through all of this, I've realised that I have fallen more and more and even more deeply in love with you than ever before. Even though things have not turned out exactly as we hoped, it still feels surreal to imagine you not being by my side. For the past three years, I have thought of you as my endgame. The person I would share my dreams, my life, and my future with.`,
    `As much as it's hurtful, I think making you happy is on the top of my list, and that would trump anything which comes in between. So letting you go and seeing you leaving, as much as it hurts to even say this, is I think the best thing I can do to keep you happy even if it means I am not gonna be there to see that.`,
    `You have changed how I love people around me, and how I talk to people. How I carry myself. And how I see the world. I see them with a nuance now.`,
    `And I definitely get that all those changes are not enough to live with you, as they are not yet to the mark, and I really hope I could be that. But unfortunately, I am slow in that, and it's a shame that you wouldn't see the person who you changed me to be.`,
    `I right now would do all the things I have been doing for a month now, as this journey has taught me, not 100%, but it has definitely taught me how to love you. What you want and why you want them.`,
    `I sometimes might not be a person who does everything right all the time, but I'm a person who would listen to you. And if you even so mention something that you like and I don't have the means, I would wait for a couple of years and do that thing for you. But never forget what you want, what you like, what you love.`,
    `I know that this letter has been full of mistakes and doesn't make sense most of the time because it's a collection of things I have been saying over a period of two to three months, and I have just put them together.`
  ]
};

export const CHAPTERS = [
  {
    num: 1,
    romanNum: 'I',
    shortTitle: 'THE MEETING',
    title: 'How it began',
    dates: 'February 2023 — September 18, 2023',
    teaser: 'A Facebook message. A closed coffee shop. A drive home the wrong way. And months of daily talk that built into something neither of them could name yet, until September 18th.',
    heroImg: '/paintings/chapter-01-hero.jpg',
    posterImg: '/paintings/chapter-01-hero.jpg',
    tint: 'reader-tint-1',
    prose: [
      `There was a guy who genuinely didn't know what was coming for him in Ireland. All he wanted, going in, was to study there, live there a couple of years, maybe more, and figure the rest out later, India or Ireland, he hadn't decided. He'd just come out of a long relationship and wasn't looking for another one. He wanted to be alone, actually alone, and that was a real part of why he took the room at Proby in the first place.`,
      `Then there was a girl he met on February 18th, 2023, on Facebook. He was scrolling through a newly created group for the UCD 2023 intake, not looking for anything in particular, when he came across a post from someone in Chennai who was going to study HRM at UCD. He never really understood, even after, why he messaged her. But that was the start of it. That was where everything began.`,
      `They just started talking. Nothing much happened at first, no big spark, just the strange comfort of already knowing someone before he'd even set foot in the college he was moving countries for. By June, they made a plan to meet.`,
      `He was nervous about it in a way that surprised him. Should he even go. Should he bring a friend along just in case, or go alone. He'd been talking to this girl for a while now and had no idea what he was actually supposed to do about it. In the end there wasn't really another option. He'd go.`,
      `She gave him three places to choose from, left the decision to him, and he picked one. They settled on a coffee shop and agreed on six in the evening. He showed up an hour early, keyed up in the way you are before something you've been thinking about for weeks, and found the place shut. Closed for good, as it turned out, closed long before that day. He called her to let her know, feeling a little foolish standing outside a shuttered coffee shop. She didn't pick up the first time. He tried again, and this time she answered, telling him she'd only just left home. So they switched to her second option instead, and he went and waited there.`,
      `When she finally arrived, he couldn't stop talking. Nerves, mostly, the specific kind that comes from meeting someone new after weeks of only knowing their voice and their texts, wondering the whole time what she made of him. He talked and talked. She was a little reserved in person, more guarded than he'd expected, but he kept going anyway, and she stayed and listened.`,
      `When it was time to head home, she offered to drop him. He hadn't had a girl drive him anywhere in a long time, and something about it caught him off guard, in a good way. She was meant to drop him at Vadapalani station. Instead she drove past the flyover and let him off in Ashok Nagar, further than she needed to. He noticed that, filed it away, this strange, funny contradiction in her, someone who came across strict and serious and was, underneath that, quietly thoughtful in a way that didn't match the first impression at all. An unusual combination, he thought, heading home that night.`,
      `They kept talking after that. That's really where it started. That same night, Chennai had been brutally hot all day, and then out of nowhere, around four, it poured. He checked his phone and found a text from her. He replied, and she replied back almost immediately, which surprised him enough that he said something like, since when do you actually reply this fast. She asked what had happened on his end. He told her his AC had stopped working and he'd woken up freezing, which wasn't really true, he just wanted an excuse to keep talking to her.`,
      `From June 3rd, they just started talking every day. Not the kind of talking two people do when they're being polite about staying in touch. Every single night he'd wake up at some random hour and grab his phone, and she'd already be there. Online. He never had to check if she was up. She just was. So they'd talk, at 2am, at 4am, whenever his body decided to wake him, because some part of him already knew she was worth losing sleep over.`,
      `Somewhere in those weeks, her dog died. Kaizen. It hit her hard, harder than she let on at first. One night she wanted to talk about him, needed to, and Aadi changed the subject. He meant well. He thought he was protecting her from sitting in it any longer than she had to. It didn't land that way. She wanted to be heard, not redirected, and it turned into a small fight. Not a big one. But real. It taught him something. How much she'd loved that dog, and how much more emotionally grounded she was than he'd given her credit for.`,
      `A few weeks later a mutual friend planned an evening at Phoenix Mall. Krithika showed up late, she was shopping somewhere else, unbothered by the time the way she usually was. When she got there it was fun, and then their friend had to leave early. First time it was just the two of them. Four in the evening turned into ten at night before either of them noticed. He felt good that day. Like nothing was in the way.`,
      `A few nights after that, he was in Coimbatore, his last night in India before Dublin. He went out for a drink with his friend Siddharth, who brought two others along, and it turned into more than a drink. He smoked for the first time that night. And somewhere in there, with more confidence than he had any business having, he asked Krithika out.`,
      `He woke up the next morning mortified. Not because he'd asked her. Because he remembered saying it out loud, and now he had to live with that. He spent the whole morning apologizing to her, not for meaning it, but for saying it. She let him squirm for a bit before letting him off the hook, and somewhere in that whole awkward exchange, something actually started between them.`,
      `The real reason he was in Coimbatore was to get his IDP, and he still went and did that, still fumbling through half a conversation with her the whole time. She handled it all with a grace he'd only understand later, because she was doing it while dealing with her own grief and never once made him feel like a burden.`,
      `By August 1st, whatever got said drunk and apologized for sober had turned into something real. Something they weren't pretending away anymore.`,
      `A couple days later he found a reason to see her again. Shopping, at Lifestyle in Ramapuram. That day he kept telling himself they were just friends, that this was fine, that she seemed fine calling it that too. He couldn't make himself believe it. She nearly stumbled reaching for something on a rack, and he caught himself staring, not at the almost-fall but at her, and that was it. He'd fallen and there was no taking it back. A bit later she went to check out a pair of pants, and he noticed her, properly, for the first time. Couldn't take his eyes off her. First time he'd ever looked at someone like that. He's told her this more times than either of them can count since.`,
      `Twenty five days later, on August 28th, he landed in Dublin and threw himself straight into time with his friends there, the way you do when you're trying not to think too much about who you left behind.`,
      `On September 1st, Krithika landed in Dublin too. She was exhausted from the flight and quietly overwhelmed underneath it. That evening his friends were cooking at his place, and there was a plan to go watch the full blood moon down at Blackrock with everyone. He called her, invited her over. She said yes, even though part of her just wanted to sleep, because the other option was being alone on her first night in a new country, and that felt worse. So she came, and the whole group walked down to Blackrock beach pier and stood there under a moon gone rust colored, and somewhere in the middle of everyone else's conversation, he and Krithika found their own. They talked for hours. About things neither of them had said out loud before. When it was time to go he walked her home, and because she was still feeling that loneliness, he stayed on the phone with her until she fell asleep. She told him the next day it meant something.`,
      `A few days after that came IKEA, which should've been nothing. A boring errand, flat pack furniture, a long walk through a maze of showroom rooms. He went early with his friends and waited for her. The second she showed up, the whole trip changed. It turned into an afternoon of laughing more than shopping. On the way home they shared one pair of earphones, trading songs back and forth, and at some point she fell asleep on his shoulder. First time that happened. He didn't move. Kept a hand resting on her head so she wouldn't slip, the rest of the way home. Next day, again, she told him it stayed with her.`,
      `Then came the Sivaji night, the one they'd been talking about for weeks. He wasn't that invested going in, but it happens to be one of his favorite films ever, and she'd finally asked to watch it, so she came over. Somewhere between the opening and the interval they were both laughing more than watching, and by the time either of them checked the time it was late and they'd only made it halfway. She stayed the night.`,
      `Before he left for class the next morning, he folded her clothes so she wouldn't wake up cold. She ended up wearing his t-shirt instead. That evening she came back and they finished the second half of the film.`,
      `For the next four or five days she was basically always at his place. Nobody said it out loud, it just became how their days worked. By then he'd stopped fighting whatever this was. They were sharing a bed most nights, and he was carrying, quietly, without telling her, just how deep it had gotten for him.`,
      `On September 15th, Krithika's birthday came around, and he realized he had no idea what she liked. He messaged her best friend Kavin on Instagram hoping for some ideas. Before Kavin even replied to him, she'd already told Krithika about the message, and the surprise was ruined before it started. He went ahead with it anyway. It happened at his place, the Proby residence in UCD, Blackrock. He set everything up and had her wait in the living room, and when she finally walked in, there was a chocolate cake waiting and photos he'd printed just for the day. To this day he still doesn't know exactly how she felt walking into that room. Whether it landed the way he hoped, or just as a nice gesture on a hard day. He's never asked her straight out, and she's never quite told him.`,
      `The next day, completely by coincidence, his friends showed up at the house, and the birthday turned into a night of drinks with everyone around.`,
      `By then there was a running joke between them, had been for a couple of months. Whenever it felt like they were about to kiss, whenever the air between them shifted even a little, Aadi would get up and go brush his teeth. Wanted to be sure his breath was good, just in case this was finally the moment. Krithika teased him every single time. Called him ridiculous for taking something so small so seriously.`,
      `Monday morning, September 18th, they were both getting ready for class, moving around the same small space like they'd been doing for weeks. The joke played out one more time. He went and brushed his teeth, same silly ritual as always. She teased him for it, same as always. Right up until neither of them was laughing anymore.`,
      `They kissed.`,
      `It's still, to this day, one of the best moments of his life.`
    ],
    tobcontinued: 'To be continued.'
  },
  {
    num: 2,
    romanNum: 'II',
    shortTitle: 'FIRSTS',
    title: 'Everything for the first time',
    dates: 'September 18, 2023 — February 14, 2024',
    teaser: 'The morning after the kiss, and everything that followed for the first time. First I love you. First Christmas nursed through fever. First official date. And the ask.',
    heroImg: null,
    posterImg: null,
    tint: 'reader-tint-2',
    prose: [
      `After the kiss, he was on top of the world. Genuinely thought, right then, that this was the best moment of his life so far, and he walked to class happy in a way he couldn't remember being in a long time. On the way there, she cut him off near the main door and kissed him again. Hot, unexpected, and honestly a little dizzying. Neither of them brought it up for the rest of that day. It just sat there between them, unspoken.`,
      `The next morning they finally talked about it properly. What is this, what's going on between us, that kind of talk. She was in a place where she genuinely didn't want anyone right then. He knew, without needing to think about it, that he only wanted her. They talked it through and landed somewhere simple. Exclusive. Girlfriend and boyfriend. Dating. And that was that.`,
      `She went off to class after. He couldn't say the same for himself. He wasn't really there that day, mentally at least. Could only think about one thing, and it wasn't whatever was on the whiteboard. Within a short while they were properly going out, having a genuinely good time being an actual couple instead of whatever careful thing they'd been circling for months.`,
      `A few days in, they went to a party with friends. Had a great night, came home, kept going out, kept having fun. They were always late to things, every single time, because they'd get sidetracked looking through photos together and end up cuddling instead of leaving.`,
      `There was a job fair on September 27th, and that morning it started raining right as they were getting ready to leave, around 10am. They went anyway, took photos before heading out, got a little wet on the way there, and came home afterward just happy to be together, rain and all.`,
      `Somewhere in this stretch a small ritual started. He'd cut and shape her eyebrows, and she'd do his. She already knew exactly what she was doing from the very first time, no fumbling, no learning curve, she just knew. And every so often, when she asked, he'd oil her hair for her, just because she liked it, and afterward they'd shower together. He'd go out with his friends some nights, she'd go out with hers, but they always found their way back to each other. Slowly, without either of them announcing it, they started moving in together.`,
      `Then Krithika's friend Praveen had a birthday party, and Praveen invited both of them, since he was a close family friend of hers. In front of everyone they played it as friends. Somewhere in the middle of the night they ended up kissing anyway, and it was thrilling in a way that snuck up on both of them. The next day was cooking together, having fun, sleeping late, talking for hours, and then a real fight. A big one, not a small one. He knew if he didn't say something first, it would just sit there unresolved, so he made the first move. Bought her flowers for the first time in his life, a bouquet, on October 11th. After that things went back to their usual shape, sleeping late, waking late, cooking, being together, falling for each other a little more each day.`,
      `On October 18th, his friends had a drinking night going, and they all ended up down at Blackrock pier. That's where, completely unplanned, with no thought behind it at all, he told her he loved her for the first time. One month into actually dating, and he wasn't even thinking about whether she'd say it back. He just needed her to know. The feelings were real and strong and he knew, even then, they weren't going anywhere.`,
      `The next morning was the release of Leo, and he went to the early show without her. She ended up on a call with her friend Kavin and started feeling like she'd missed out on something she wanted to be part of. So they rebooked tickets and went together that evening instead, and a couple of days after that they tried to go again, this time running characteristically late, but it didn't matter. They took photos, kissed while taking them, showed up whenever they showed up. First time they'd properly gone to the movies together like a real couple. From there, pretty much every day fell into the same rhythm. Being together, talking, being intimate, just enjoying each other fully.`,
      `On October 29th they went clothes shopping together for the first time. A Sunday, good weather, sunny. They took photos while they were out. By the time they got to the shop they only had about an hour before it closed, and that was fine too. It was never really about how much time they had. It was about having it together.`,
      `November 3rd was their first unofficial date. Coffee in Blackrock, then down to Blackrock beach for a photo and some time together. Not long after that, something heavy happened. A pregnancy, and it ended in her having to get an abortion. One of the harder things they went through as a couple. Aadi didn't handle it as well as he should have in the moment. Real misunderstanding on his end, and he didn't give her the support she needed when she needed it most. It's not something that gets smoothed over easily, and it wasn't. But somehow, together, they got through it.`,
      `Before all of that, there'd been a Diwali event, Aadi's cousin's wife had invited them along. They were meant to be there by ten and didn't leave the house until noon, showed up anyway, ate proper Indian food for the first time in a while, and made the hour and a half journey home together. It was a good night. He loved every part of being there with her.`,
      `Then came first trimester exams. They studied hard, technically, but mostly they were just wrapped up in each other. On December 10th, 2023, she said "I love you" back for the first time. After that, he wasn't the same person. Couldn't stop thinking about her, completely hers from that point on. That whole stretch of December was studying, eating, sleeping, being together, being intimate, doing next to nothing and somehow it was everything.`,
      `Then on December 22nd he got seriously sick. Bedridden, couldn't get up for days. Krithika took care of him for ten straight days, December 22nd through January 1st. Bad enough that some nights she had to sleep outside the room just to get away from how badly he was snoring. Through all of it, she still gave him the first bouquet anyone had ever given him in his life, on December 27th. They spent their first Christmas and their first New Year together like that, him too sick to do much of anything, her skipping a New Year's party just to stay by his side, and him still trying, in whatever way he could manage, to make the holidays feel special for her.`,
      `On January 4th they started looking for part-time jobs, since flights to India were already booked for March 9th, 2024. They went everywhere. Dundrum, Rathfarnham, Carrickmines, Grafton Street, Dawson Street, wherever there was a chance. Even the job hunt was fun, because they were doing it side by side. Within about two weeks, both of them had jobs. Eddie Rockets in Dundrum for her, Applegreen Bakewell Deli for him. Her own place would've meant basically no commute at all, but she never went back to living there. She just kept staying with him, and her things kept migrating over bit by bit until his place was full of both of them.`,
      `They picked up a rental bike from the university, through the Proby residence, and went for their first cycle ride together. Slowly they started building the small routines of an actual shared life. Grocery runs by bike, since carrying everything on foot wasn't realistic. After finishing his own part-time shift, he'd often cycle over to meet her when hers ended, more than a couple of times, just so she wouldn't have to make the walk back alone. She loved that, the attention, the small effort of it. Plenty of movies in this stretch too. Just good, easy time together.`,
      `On February 6th, 2024, they had their first official date. Zaitoon. He brought her a bouquet and a Dairy Milk. It happened to be one of the coldest nights either of them could remember, cold enough that they could barely stand being inside the restaurant, the heating just wasn't cutting it. Didn't matter. They enjoyed every minute of it anyway, because they were together.`,
      `Then came February 14th, and he had something much bigger planned. He wanted to ask her to officially move in with him, to start looking for a place together for real. He was nervous the entire time, genuinely convinced she might say no, but he got everything ready regardless. They went to Apache Pizza first, their usual date night spot, and came home after. She was exhausted by then, but he kept her up another half hour without telling her why, just enough time to get everything set up. When it was ready, he went and got her from the living room and brought her into the bedroom.`,
      `She said yes immediately. It was magic.`
    ],
    tobcontinued: 'To be continued.'
  },
  {
    num: 3,
    romanNum: 'III',
    shortTitle: 'BUILDING',
    title: 'Building a room',
    dates: 'February 14 — October 31, 2024',
    teaser: 'Moving into Woodbine, first snow, a trip home to India, and the slow build of a real shared life in Dublin, good days and one very bad night, right up to finding Potters Hill.',
    heroImg: null,
    posterImg: null,
    tint: 'reader-tint-3',
    prose: [
      `Not long after Valentine's Day, they started actually house hunting for real. In between all of it, there were still their part-time jobs to juggle, his and hers running on different schedules. More than a couple of times, once his own shift ended, he'd go straight over to visit her at hers before heading home. Around then he also had a big presentation coming up, and she showed up to cheer him on, brought along a chocolate bar for him, and then ended up eating it herself on the way there before he ever got a bite. That was the kind of thing that kept happening between them, small and a little silly and somehow always sweet.`,
      `Eventually they settled on a place in Woodbine. Decided on it, put down the advance, and had everything squared away by February 26th, with the move itself set to happen between March 1st and 7th. Around this same stretch Krithika lost her job at Eddie Rockets, they wouldn't give her the holidays she needed, so she picked up work at McDonald's instead, all of it happening before the India trip they already had booked.`,
      `On the morning of March 1st, around ten, they got their first snow together. First snow either of them had ever seen in their lives. It's hard to describe how strange and beautiful that moment was without sounding like every other person who's ever talked about their first snow, but for them it really was something else. They went out and played in it for over an hour, took photo after photo. The one thing that still bothers him, even now, is that somehow, in all of that, they never once got a photo of the two of them together in the snow.`,
      `By March 7th, they'd fully moved out of her place and into the new one. 26 Woodbine. Their address now. With flights to India booked for March 9th, those two days in between were just packing, nothing else, since all the shopping had already been done ahead of time. They were beyond excited to finally be heading home after seven months away.`,
      `That last night before the flight was a long one. Twenty four hours in the air, landing in India on the evening of March 9th. It was the first night in a long while that they slept apart from each other, and it hit harder than either of them expected. Genuinely hard on both of them.`,
      `Being back in the same country but not the same house meant they had to get creative about seeing each other. Two or three days after landing, they met up to eat soya chops and ice cream, just the two of them, stealing whatever time they could. Within two days of that, they met again near Express Avenue Mall and snuck into an OYO for the evening, not for any reason beyond wanting to be near each other without an audience.`,
      `On March 15th, Aadi, Krithika and her friend Kavin went to a fair together, rode the giant wheel, had a genuinely fun evening. After that there was a stretch with no meetups at all.`,
      `Then came March 19th, when Aadi's parents happened to be out of Chennai. It was also their six month anniversary. The night before, Krithika stayed over at Kavin's, and Aadi used that window to plan a surprise with Kavin's help, a bouquet, meeting her before the evening properly started, a car ride, time at the beach. As romantic as he could make it. They ended up spending the whole night together at their favorite beach, and it turned into one of the best nights they'd had together in India, not the first time they'd been intimate, but one of the best.`,
      `March 22nd, they went and bought her first iPhone. He picked her up from her house, more than a little awkward with her mom standing right there wanting a photo of the two of them, one of those funny, cringing moments neither of them will forget.`,
      `Then came his birthday. They celebrated two days early, on the 26th, since the actual day was meant to be spent with his parents at home. That day was foot massages, then dressing up for a long drive, and a slow, easy stretch after, food ordered in, face masks, sleeping in late. But the actual day, his twenty fifth birthday, brought one of the worst fights they'd had up to that point. They didn't speak the entire day. He spent it just wanting to know she was okay. That evening she called him over to her place, meant to be a quick visit, and it turned into one of the best moments between them, the kind he still thinks back on during his best nights.`,
      `On April 1st they flew back to Dublin, and life picked back up. Same house, work, skipping college more than either of them probably should have, talking until five or six in the morning even with his eight o'clock shifts waiting for them. In between, they went to Hansang a few times, always fun no matter how little money they had. They ordered in, they played around, they fought sometimes, and that's just how those weeks went.`,
      `In May they picked up a projector and a Chromecast, so movie nights became a thing they could actually settle into without any fuss. There was also their first proper night out with a rented car, down to Bray Beach with their roommates Divyesh and Sanjana, coffee, a lot of photos, and it felt like a real milestone, having a car of their own to take out again after so long without one. They kept up regular day trips and evenings out to Dun Laoghaire too, even with their shifts working against them, him mornings, her evenings, never much overlap. Some of their best memories from this stretch are with Divyesh and Sanjana, in a house that was falling apart in places but that they loved anyway. Aadi had promised, before they moved in, that he'd clean the bathroom if they took the house, and eventually he did, messily, but he did it.`,
      `June 5th brought their first picnic, completely spontaneous. She'd gone to college on an unremarkable day, the weather was good, so he packed everything up and met her at Herbert Park. Simple, playful afternoon. Somewhere around here he also did the Jai meme dance and sent her the video just to make her laugh.`,
      `June 10th, their first swim together, at the UCD pool. Neither of them actually swam much, just floated around together, then moved on to the jacuzzi, steam room, and sauna, all firsts for both of them.`,
      `Then came a harder night. A fight that ran until four or five in the morning. She left the room, and instead of going after her the way he usually would, he was too tired and fell asleep instead. She cried for hours. Looking back, he calls this the start of everything that came after, and he knows she still hasn't fully forgiven him for it.`,
      `College ended in July. Life settled into a rhythm of Musashi, his part-time shifts, groceries afterward when she'd meet him, cooking, ordering in, just being around each other as much as the schedules allowed. She left McDonald's for Subway when the hours there stopped being enough to live on. One night the four of them, Aadi, Krithika, Divyesh and Sanjana, drank together for the first time as a group, and there was a trip to the rainforest mini golf course in Dundrum, just the two of them, fighting the entire way there.`,
      `Then a harder stretch when Kavin came to visit. Aadi got in his own head about not getting enough attention while she was around, let it turn into a tantrum, and ended up cutting into time Krithika wanted to spend with her friend. She eventually let it go and forgave him, though it's stayed with her as one of the harder memories from that year.`,
      `Her birthday came in September, her first as a couple with him. He rented a car and took her up to the Killakee viewpoint, brought the same style of cake as the year before with rose bouquets, then home to a surprise waiting for her, a candlelit carpet with a two kilogram cake and every one of her favorite snacks laid out. The next morning, Momos and a cooked breakfast, though she skipped the breakfast and just had the Momos. Then out for the rest of her actual birthday, where her mom had a surprise cake planned with Aadi's help, a lot of walking through the day, and King's Biryani for the birthday dinner.`,
      `The evening before their anniversary they went to Milano in Dundrum, and the plan was Cosmo, the multi cuisine buffet, on the day itself, September 19th. They never made it. A big fight got in the way, and looking back, this is the point he marks as the beginning of the relationship's harder chapter. Fights started coming more often from here, even while the good days, including the closeness between them, were still very real.`,
      `Not long after, Divyesh and Sanjana told them they were moving out to Dun Laoghaire. All four of them had been house hunting together, hoping to find something to share, but nothing came through in time. That same night, Aadi and Krithika started looking on their own, and within two days they found a place in Leopardstown, Potters Hill. The fighting kept going through the whole process, even as they signed on for it.`,
      `Their last month living with Divyesh and Sanjana was still a good one, full of nights together before the split. A Polaroid camera night. A day out in Kildare, a Rajini movie first, then a full day out in a rented car. They just wanted to hold onto what they had with them for as long as they could.`
    ],
    tobcontinued: 'To be continued.'
  },
  {
    num: 4,
    romanNum: 'IV',
    shortTitle: 'HUSTLE',
    title: 'The hustle year',
    dates: 'November 1, 2024 — June 2025',
    teaser: 'Potters Hill, graduation day, her new job at Aon, Edinburgh, and the biggest fight yet, the one that never quite stopped echoing.',
    heroImg: null,
    posterImg: null,
    tint: 'reader-tint-4',
    prose: [
      `They moved into Potters Hill on November 1st, 2024. Did it all themselves, no real help, and they fought through most of it, even the first night in the new place. That was the shape of things from the start there, laughing one minute and at each other's throats the next.`,
      `Once they were settled enough, Krithika had to get ready for her interview at Aon. It landed on November 5th, the same day Aadi had a LinkedIn event to go to. She almost didn't go, told him she felt underprepared, but he pushed her to show up anyway. She made it on time and she nailed it. Heard back within a couple of days. It was one of the happiest moments she'd had in a long while.`,
      `Graduation shopping started around then too, for the ceremony set for December 4th. Her parents were flying in, her mom on the 28th of November, her dad the day after. In between all that Aadi finally bought a PS5, something he'd wanted since he was a kid. For the first time since they'd been together, they spent a stretch apart while still in the same city, five to ten nights, him staying with his own parents at his cousin's place, her staying with hers at the Stay City Hotel in the centre. They'd both applied for UK visas for a side trip to London. His came through. Hers didn't, and she had to miss the whole London leg with her parents, which weighed on her a lot at the time. He went on to London anyway, came back, and then they bought a television together, with a fair amount of back and forth over how it should be set up. Nothing serious, just two people figuring out how to share a living room.`,
      `Graduation day, December 4th, both sets of parents there at the same ceremony, same time, lots of photos. Even mid quarrel, Aadi stepped away at one point just to grab a picture with her. After that his brother Pavi came to visit, and the two of them went off to London together for a few days before Christmas, leaving her alone in Dublin for a stretch.`,
      `Their second New Year came after Christmas. By then she'd started her new job, December 9th, desk and laptop delivered and everything. She wanted to work from the living room since she was fully remote, so she had Aadi check with their housemates, Vidya and Vishanth, and they were fine with it. On New Year's they went out to Daata in Bray, which they just call Daata Bray, and she discovered a pistachio milk cake there that she still, to this day, can't get enough of. He's made that drive out for it more times than either of them could count since.`,
      `That's when the hustle really started. Aadi's hours at the deli kept shrinking, not even enough to cover rent, and even though he'd told himself he wouldn't switch jobs inside the first year, he had no choice but to start applying elsewhere. By February 3rd he landed something at Pennys. Krithika, meanwhile, was genuinely into her new role at Aon. They celebrated their first Pongal together in January, Vidya cooked, everyone gathered around. On the whole, life was good, but the working hours had them barely overlapping, her five days a week during the day, him five evenings, with almost no time left over for each other.`,
      `Valentine's Day that year brought two gifts each, actually. He got her thigh length boots and headphones. She got him the Lego set he'd been wanting for ages. Around this time he started noticing something, that they weren't taking as many photos together as they used to. This was February, March of 2025. Still plenty of good days, movies, grocery runs, time out together, but the fights were creeping in more often underneath it all. A trip to Encha in the city centre before his birthday. On their eighteen month anniversary he tried making her the pistachio milk cake himself, got most of the way there alone, finished it together with her, and it actually turned out delicious.`,
      `Then his birthday. She gave him a guitar, made him a cake, cooked noodles and gulab jamun, and they went to Anjappar that night for dinner. A good day, all around.`,
      `April 5th brought their first international trip together, three days in Edinburgh. They'd saved up and put it together fairly last minute, he had four days off in a row, she had three. They'd already worked through a few Harry Potter films together and loved being somewhere that felt like it came out of them. They fought once, during a sunset, and it soured that night, but they went back the next evening and watched the same sunset again, and it was beautiful. The fight didn't really follow them past that.`,
      `Back home, it was straight back to the grind, almost no time together at all. His Pennys contract ended April 28th, and he flew to India on the 30th for his best friend Rishan's wedding, originally meant to stay until May 30th but ended up staying until June 12th. Krithika couldn't stand being apart from him the whole stretch, so she flew out too for two to three weeks. While she was there, they had one of the biggest fights of the relationship, real breakup talk, ugly in a way that neither of them had let it get before. That's the point he marks, looking back, as where things really started to shift and never fully came back. Even on the good days after that there was something underneath it now. They barely spoke for a week or two. On May 23rd they tried to reset, went down to the beach together, watched the sunrise in Chennai. It helped, a little. Still broken, but not past fixing. Not yet.`,
      `She flew back to Ireland first. He stayed on another two or three weeks. They missed each other, but it wasn't the same pull it used to be, the conversations had lost some of their old strength. By early June, he came back too, with no job waiting for him, burning through what savings he had left, applying to anything he could find.`
    ],
    tobcontinued: 'To be continued.'
  },
  {
    num: 5,
    romanNum: 'V',
    shortTitle: 'THE STRETCH',
    title: 'The long stretch',
    dates: 'June — December 6, 2025',
    teaser: 'A hike that left something behind. A stretch of low grade fighting that never let up. A second anniversary in Waterford. And finally, a place of their own: Laurel Hall.',
    heroImg: null,
    posterImg: null,
    tint: 'reader-tint-5',
    prose: [
      `On June 21st they went on the Bray hike, and it turned into one of the worst fights they'd had. The kind that doesn't just pass, the kind that leaves something behind. He said things that day, and things happened after it, that he's never forgiven himself for, and that she's never fully let go of either. Looking back, this is the point they both quietly understood that things weren't going to get better anytime soon, even if neither of them said it out loud yet. Around this same stretch they started working out together, trying to build something steady in the middle of everything else falling apart. He was in a genuinely bad place then. No full-time job, no part-time job, savings running thin, applying anywhere he could.`,
      `By July he finally landed something at Spar in Blackrock. The fighting kept going regardless, frequent, exhausting. They still made it to the beach together fairly often, actual swims, not just sitting on the sand, and sometimes Boojum for food. Money was tight enough that Aadi stopped planning dates altogether, which became its own quiet wound between them. They did make it out properly a couple of times, Jury's for a theatre performance, an Italian place after, evenings that reminded them both of what things used to feel like. But the stretch overall was rough. Constant, low grade fighting that never really let up. By August, Krithika told him outright that she resented him, that things were never going to go back to how they used to be. He held onto hope anyway. It didn't really pan out.`,
      `There were good pockets in there too. Some weekends she'd go dog sitting, and he'd come by after his shift just to be with her there, away from everything else. Her second birthday together as a couple, he planned it carefully, bowling in Bray, then Daata for food and that pistachio cake she loves so much. A good day, even if it didn't manage to pull the bigger picture back together. Around this time he introduced her to How to Train Your Dragon: The Light Fury for the first time, and it became one of their things.`,
      `September 19th, their second anniversary, they went to Waterford and Wexford for kayaking. A high end dinner that night, her treat, both of them dressed up, a cozy Airbnb in Waterford, a drive, real closeness that night, kayaking the next morning. A beautiful date on paper. The arguments kept finding their way in anyway, even around days like that one.`,
      `Toward the end of September, Vidya and Vishanth told them they'd be moving out. This time, when Krithika brought up breaking up, her third or fourth time saying it, she meant it differently. She started actually looking for places, and asked Aadi to do the same. They couldn't find anything for weeks. Tried for temporary places too, in case nothing came through in time, and came up empty there as well. All through the search, the fighting continued, and Krithika was worn down most days, just wanting some distance from him, which broke something in him to watch happening and know he couldn't fix quickly.`,
      `Finally, on November 15th, they got their own place. Laurel Hall, in the same Clay Farm community they'd been living in at Potters Hill. Move in was set for December 6th. It was a hard, fight filled stretch right up to and through the day itself, even as they moved.`
    ],
    tobcontinued: 'To be continued.'
  },
  {
    num: 6,
    romanNum: 'VI',
    shortTitle: 'ANYWAY',
    title: 'Inevitable, but anyway',
    dates: 'December 6, 2025 — July 2026',
    teaser: 'Laurel Hall, her surgery and recovery, a new job, a pottery afternoon, and a fight bad enough that they started taking photos separately again.',
    heroImg: null,
    posterImg: null,
    tint: 'reader-tint-6',
    prose: [
      `Right around the move, Spar stopped giving Aadi enough hours too, so he switched over to SuperValu, and ended up having to move house the exact same weekend he started the new job. Hectic doesn't quite cover it. Lots of fighting, lots of laughing in between the fighting, and somehow they got through the move itself. Even after they were in, he still had things to bring over from the old place, and it dragged. She kept asking him, over and over, when he was actually going to finish moving out, because he'd promised her once they had their own place he'd be the one to go, not make her wait on him. By then he knew, without needing to be told, that she'd been resenting him for a long while.`,
      `Christmas at Laurel Hall was one of the better nights. They planned a proper cooking night together, parottas, salna, chicken roast with Bellies on the side, exchanged gifts, laughed a lot. A good day, simple and warm.`,
      `Then came a trip built around her friend's wedding in India, two to three weeks, and Aadi couldn't stand the idea of being apart that long, so he planned his own Pongal trip around the same window. They flew separately. Once there, they barely talked, she was busy with the wedding, he was busy with his own short trip. Still, they managed something sweet in the middle of it, their third Valentine's Day, celebrated early in Chennai on January 25th, since Krithika was scheduled for achalasia surgery and wasn't sure she'd be back in time for the real date. It was a good, low conflict stretch while they were both away.`,
      `Aadi flew home January 29th. She stayed behind for her surgery and recovery, three months in total, which was genuinely difficult for him, being apart that long again. She came back March 20th. In between, he leaned hard into work, it was really the only thing keeping him steady. They talked every night, and there was almost no fighting, except for one night when she sank into a real low, questioning what she was doing with her life, telling him she didn't want to be with him anymore, that he wasn't the right person for her, that she was certain by then he wasn't going to change. He tried to talk her back from it. It didn't really work. Things felt stuck after that, good conversations that never quite turned into good momentum.`,
      `By the time she was back in Dublin, Aadi had landed a full-time job at Teleperformance. They wanted to celebrate properly but he didn't have the money for it, so they folded the celebration into his birthday instead, since the two landed close together. She gave him a custom newspaper she'd written herself, entirely about him and their life together, something he still calls one of the most meaningful things anyone's ever given him. At the time, though, he didn't show her how much it meant the way she needed him to, and she told him later it had hurt her that he hadn't. When she finally brought it up, it broke something in him to realize he had appreciated it deeply and just never let her see that. She made him biryani that day, dressed up beautifully that evening, and they went to a Lebanese place, her treat, marking both his birthday and the new job. Even with the relationship worn thin by then, she still worked hard to make that day feel good for him. As for the job itself, it paid better than anything he'd had before, but the schedule was random, no fixed days off, which meant even less real time together than they'd already been losing.`,
      `Life kept moving after that, still full of fights, still together, but never settled. Underneath it all, Aadi knew, had known for a while, that this wasn't going to be the one that made it all the way. Even so, there were good days. A pottery class they spent an entire afternoon on. Dinner at Big Fan, a Michelin recommended spot. A café after with mango, raspberry, strawberry, and pistachio shaped desserts. A Thai massage before all of it. Days where they could spend freely and enjoy themselves without thinking too hard. He tried to change the things she'd complained about, the lack of planning, the lack of effort on dates, and he did try. But she never fully believed it, never came back around to feeling like things were the way they were supposed to be. From that point on, some part of her stayed convinced of that. They went to Shasta by the City in Central Park. They went to Pantry in the Leopardstown shopping centre. Good dates, some of the best on paper, but neither of them could picture the two of them together a year or two out anymore.`,
      `By May, things hadn't improved. If anything, they'd both quietly settled into being sure they weren't right for each other, even without saying it plainly. There was a picnic, more drinks, and then a dinner at Vanilla Pod in Carrickmines, meant as a treat for her bonus. That night ended in one of the worst fights yet, bad enough that she was ready to move out right then. He remembers that night vividly, partly because of something small and painful, they still took photos, just separately now, him taking hers, her taking his, instead of together like they used to. It was getting harder and harder to keep going, but they kept going anyway, still on borrowed time, because there was already a plan in motion, he was heading to Chennai for a friend's wedding and his mom's fiftieth birthday.`,
      `The two weeks he spent in Chennai were genuinely good. They didn't fight much. He was enjoying himself, she was focused on her own work back home, and when they did disagree it passed quickly. She flew out to join him near the end of the trip because he had a Salesforce interview coming up, and she understood that fighting in that window would hurt his chances, so she put her own feelings aside to be there for him instead. He's deeply grateful to her for that, even now. He came back to Dublin on June 29th, and for a little while it felt like things might actually turn a corner, like the break had given them both something they badly needed.`
    ],
    tobcontinued: 'To be continued.'
  },
  {
    num: 7,
    romanNum: 'VII',
    shortTitle: 'THE END',
    title: 'Where it ends',
    dates: 'July 2026 — present',
    teaser: 'The fight that finally settled it. Cork, and the conversation that said what neither of them could take back. Still sharing a home, still waiting on Iceland, before it ends for real.',
    heroImg: '/dragons/scenes/chapter-07-mark.png',
    posterImg: '/dragons/scenes/chapter-07-mark.png',
    tint: 'reader-tint-7',
    prose: [
      `By the second week of July, whatever hope had come back with him from Chennai was gone again. They had a fight that week unlike the others, not louder necessarily, but final in a way the earlier ones hadn't been. It was the fight that actually settled it, the one where the decision stopped being something they circled around and became something they'd both said out loud and couldn't take back. They knew, from that point, that this was ending.`,
      `They didn't move out right away. There was too much already set in motion to walk away from cleanly, an Iceland trip they'd been planning since March, already booked and paid for, and a trip to Cork on the 20th and 21st of July that had been on the calendar for a while too. So they set a timeline instead. They'd stay together, in the same place, until after their third anniversary, and then it would be over. The ending was decided long before it actually arrived.`,
      `They went ahead with Cork anyway. It wasn't a trip either of them enjoyed, not really. But the first night there, they drank more than usual, and somewhere in that they had one of the most honest conversations they'd had in a long time. They agreed, plainly, that they weren't good for each other. Aadi told her he wasn't going to be the right person for her, not the version of him she needed, not the version he'd tried and failed to become. It wasn't one conversation. It stretched over two or three days, the same ground covered again and again, until it settled into something neither of them could argue with anymore. Staying together was only going to hurt her more. Whatever changing he'd tried to do, it wasn't enough, and it wasn't going to be enough going forward either. Something in her had changed too, by then, past the point of coming back. The resentment had built into something closer to genuine dislike, and he could feel it even when she wasn't saying it directly.`,
      `The conversation in Cork was deep, and in its own strange way it was good, one of those talks that lets two people finally say the true thing after months of circling it. But it wasn't a fight that fixed anything. It was bittersweet more than anything else, a chance to look back on the whole relationship and see clearly, for the first time in a while, how much better it could have been if he'd changed sooner, changed more, changed in the ways that actually mattered to her.`,
      `After Cork, it was mostly downhill. Not much going out anymore. She took a trip to Turkey not long after, and strangely, the time apart was good again, the way it always seemed to be when they weren't actually in the same room. They messaged, missed each other, started to feel like maybe they wanted to try being together after all. And then they were together again, in person, and the fighting came right back with them, like it had just been waiting.`,
      `That's where things sit now. They are still, technically, together, sharing a home, because the practical shape of their lives hasn't allowed for anything else yet. The Iceland trip is still coming. There's a trip to Italy already planned. A Pradeep Kumar concert they're both going to. Krithika has her own trip to India coming up too, and between all of it, and not yet having found a new place to live, there hasn't been room to actually separate. So they don't bring things up. They try to be kind to each other, careful with each other, the way you are with someone you still love but know you can't stay with. It isn't the relationship continuing. It's two people trying to make the most of what's left of it before it has to end for real.`,
      `The truth underneath all of it is simple and it's not easy to say. The love never went anywhere. It's still there, on both sides, even now. It just isn't enough anymore to make the day to day work, and they both know it, and neither of them is pretending otherwise. The home they've built together carries a kind of sadness in it these days that wasn't always there, a quiet the size of the plans still ahead of them, plans that were once about a future together and are now just the last things they'll do as two people who loved each other completely, before they finally let each other go.`
    ]
  }
];

export const CAST = [
  {
    id: 'aadi',
    name: 'Aadi',
    role: 'the one who wrote this',
    dragon: '/dragons/dragons-login.png',
    dragonSide: 'dark',
    bio: 'Called Ammu. Chennai to Dublin, June 2023 onwards. Says I love you more than she does. Says porikki less than she does. Wrote every word here.'
  },
  {
    id: 'krithika',
    name: 'Krithika',
    role: 'the one it is for',
    dragon: '/dragons/dragons-login.png',
    dragonSide: 'light',
    bio: 'Also called Ammu. Says porikki more than any word in her vocabulary. Signs off with miss you love you like it is one word. Reads this now.'
  }
];

// Nagumo removed from the list (Aadi: not a song I want in the list — said
// again, emphatically, when re-dictating these notes for v4.6).
export const SONGS = [
  {
    n: 1,
    title: "Sai Pallavi's Intro",
    from: 'Amaran',
    artist: 'G. V. Prakash',
    timestamp: '0:10',
    trackId: '7tr5DrOhoA4Dolg8X1OnRQ',
    note: "That is you Kannama for me, that is the life I wanted that is the music you are to me. You are that."
  },
  {
    n: 2,
    title: 'Hey Minnale',
    from: 'Amaran',
    artist: 'G. V. Prakash, Haricharan, Shweta Mohan, Karthik Netha',
    timestamp: '1:26',
    trackId: '1kmJ0EvXClJAt0fSSQLX1m',
    note: "The meaning of the entire song is how I feel about you. The song may not be perfect, but you are."
  },
  {
    n: 3,
    title: 'Konji Pesida Venaam',
    from: null,
    artist: 'Nivas K Prasanna, K. S. Chithra, Sriram Parthasarathy',
    timestamp: '1:56',
    trackId: '5QoCBy9eUHYkeWpEpzAud9',
    note: "That is our song, like our song whenever I try to be playfully pull you closer to me when you are pissed or whenever I try to be naughty with you. It's just, after a fight, after a small quarrel, looking at you and singing this song gives me everything."
  },
  {
    n: 4,
    title: 'Kurumugil',
    from: null,
    artist: 'Vishal Chandrashekhar, Sai Vignesh, Madhan Karky',
    timestamp: '1:21',
    trackId: '4DKv9524rd34cCKFmW7Xsb',
    note: "This is one of the most romantic and every single time I look into your eyes, this is a song I want to sing to you."
  },
  {
    n: 5,
    title: 'Kannukkulle',
    from: null,
    artist: 'Vishal Chandrashekhar, Haricharan, Sinduri Vishal',
    timestamp: '1:37',
    trackId: '6O1aZHTKj8bKhb3N5qB6lF',
    note: "This song just reminds me of the times we used to dance together. I used to just hold your hips and, not dance exactly, but just vibe to it and look into my eyes. That is all I think about when I listen to this song."
  },
  {
    n: 6,
    title: 'Kannal Pesum',
    from: null,
    artist: 'S. P. Balasubrahmanyam',
    timestamp: '0:00',
    trackId: '4VE07nOfv8CXbmRcubchO6',
    badge: 'the apology song',
    note: "This is the first ever song I sang to you. This will always be my apology song. Hasn't been that effective recently, but this is one of the reasons you partly fell for me. But yeah, I think it is a good thing that it stopped working because we would still be together if it was working, you know. Thank goodness the apology song is not working."
  },
  {
    n: 7,
    title: 'Nenjam Ellam',
    from: null,
    artist: 'A.R. Rahman, Adnan Sami, Sujatha, Vairamuthu',
    timestamp: '1:43',
    trackId: '5lmgII1wyydsuqXEOvzpSy',
    note: "This has been our sex song. Nothing but just the times we made love when we were staying in Proby. This is the only thing which reminds me of those sweet, sweet times."
  },
  {
    n: 8,
    title: 'Until I Found You (with Em Beihold)',
    from: null,
    artist: 'Stephen Sanchez, Em Beihold',
    timestamp: '1:21',
    trackId: '1Y3LN4zO1Edc2EluIoSPJN',
    note: "This is the song, if you remember, this is the first song I ever dedicated to you. And funny thing, this is our climax and this is how we are. At least I am."
  },
  {
    n: 9,
    title: 'Sahana Saral Thoo',
    from: null,
    artist: 'Udit Narayan, Chinmayi, Vijay Yesudas',
    timestamp: '1:16',
    trackId: '317JrLcl1s0i6cXmnEWzR8',
    note: "This song is the most number of times I've ever, like, ever looked into your eyes and have sung this song. My God I cant resist myself"
  },
  {
    n: 10,
    title: 'Innum Konjam Neram',
    from: null,
    artist: 'A.R. Rahman, Vijay Prakash, Shweta Mohan, Kabilan',
    timestamp: '4:37',
    trackId: '5KJSI1MjQeXEBHBYMjgP41',
    note: "This song was first when we were walking in the beach and we were singing. And this song came up and we talked how we wanted our lives to be. But then it is not. It is sad, but it feels good to think about all those times with you. you even said those lines about the kids to me, but I would never be worthy of it."
  },
  {
    n: 11,
    title: 'Aval',
    from: null,
    artist: 'Santhosh Narayanan, Pradeep Kumar, Priya Hemesh, Vivek',
    timestamp: '0:47',
    trackId: '4LK8863ScRhhT6hitFfBI5',
    note: "Every time I wanted to apologise to you to start again but never ever cuz I thought I would never have to ask that. But never got to and here we are sharing last few moments of our life together. Didn't sing these lines to you but means everything"
  }
];

// Track 13 lives on its own page — the Final Song
export const FINAL_SONG = {
  trackId: '3oNVqllTnz7bHrY3f0nICg',
  title: 'Main Phir Bhi Tumko Chaahunga',
  label: 'the closing note',
  prose: `Exactly at 26th second of the songs. These are some lines from Main Phir Bhi Tumko Chahunga that I don't think I will ever be able to hear the same way again: "Aise zaroori ho mujhko tum... Tere darr pe hi reh jaaunga." I want to dedicate these lines to you because after everything we've lived through, they don't feel like lyrics to me anymore. They feel like something I have to actually live from the next week, the day we officially are no longer together. You became part of the smallest things in my life without me even noticing when it happened, from visiting me for an evening till being my partner for 3 years. The person I wanted to tell things to first, the person whose face I looked for when something funny happened or to be fair, anything happened, the person whose toothbrush was beside mine, whose things were around the house, whose name became part of my everyday conversations. Three years of memories don't disappear just because we decide that we can't continue. I can accept that we need to let each other go, but I don't know how to make my heart understand that the person who was part of almost every version of my life for the last three years is suddenly supposed to become someone I don't get to have anymore.

And that's what makes these lines hurt so much. "Main phir bhi tumko chahunga." I don't mean that as a promise that I'll wait for you or that I want you to come back or even that I can't accept what we're doing, I swear I have no intention at all of asking you to stay, I have seen enough of me hurting you and I can't do it ever again. I mean that even after we leave each other, there will still be a part of me that loves you. I'll remember the girl who first made me nervous, the girl I waited an hour early to meet, the girl who fell asleep on my shoulder, the girl I travelled with, fought with, laughed with, built a home with, and somehow grew up beside. I'll remember the good days, the days I wish I could change and the days that if I had the power to absolutely erase from our memories. And maybe one of the hardest things about letting you go is knowing that I can't actually erase any of it and I have to the limit used up all my retries. I don't want to forget you just so this hurts less. I would rather carry the pain of having loved you than pretend these three years didn't change me. So if one day these memories hurt less for you than they do for me, I hope you still remember that there was once someone who loved you so deeply that even when he finally understood he had to let you go, he couldn't make himself stop loving the life he had with you.`
};

export const SPOTIFY_PLAYLIST = 'https://open.spotify.com/playlist/3VJ9qmmguSXJ1SdR5YaqWg';
export const SONGLINK_PLAYLIST = 'https://song.link/https://open.spotify.com/playlist/3VJ9qmmguSXJ1SdR5YaqWg';

export const FILMS = [
  { name: 'Eternal Sunshine of the Spotless Mind', note: null },
  { name: 'Aaromaley', note: null },
  { name: 'Theeran Adhigaaram Ondru', note: null },
  { name: 'Saba Nayagan', note: null },
  { name: 'Harry Potter and the Philosopher\'s Stone', note: null },
  { name: 'Harry Potter and the Chamber of Secrets', note: null },
  { name: 'Harry Potter and the Prisoner of Azkaban', note: null },
  { name: 'Harry Potter and the Goblet of Fire', note: null },
  { name: 'Harry Potter and the Order of the Phoenix', note: null },
  { name: 'Harry Potter and the Half-Blood Prince', note: null },
  { name: 'Harry Potter and the Deathly Hallows – Part 1', note: 'all 7 in a row' },
  { name: 'Harry Potter and the Deathly Hallows – Part 2', note: null },
  { name: 'Run', note: null },
  { name: 'Viduthalai Part 1', note: null },
  { name: 'Viduthalai Part 2', note: null },
  { name: 'Aavesham', note: null },
  { name: 'Mouna Ragam', note: null },
  { name: 'Geetha Govindam', note: null },
  { name: 'Veetla Vishesham', note: null },
  { name: 'Pannaiyarum Padminiyum', note: null },
  { name: 'Thozha', note: null },
  { name: 'Tamizh Padam', note: null },
  { name: 'Laapataa Ladies', note: null },
  { name: 'Premalu', note: null },
  { name: 'Kappal', note: null },
  { name: 'Koozhangal', note: null },
  { name: 'Mark Antony', note: null },
  { name: 'Jigarthanda Double X', note: null },
  { name: 'Chiththa', note: null },
  { name: 'Lover', note: null },
  { name: 'Blue Star', note: null },
  { name: 'Irugapatru', note: null },
  { name: 'Theera Kaadhal', note: null },
  { name: 'Fair Play', note: null },
  { name: 'Leo', note: 'the walk from Proby to Frascati in the storm, then 45 minutes late' },
  { name: 'The Goat Life', note: null },
  { name: 'Manjummel Boys', note: null },
  { name: 'Parking', note: null },
  { name: 'Kaathal – The Core', note: null },
  { name: 'How to Train Your Dragon', note: null },
  { name: 'How to Train Your Dragon 2', note: null },
  { name: 'How to Train Your Dragon: The Hidden World', note: null },
  { name: 'How to Train Your Dragon: Homecoming', note: null },
  { name: 'Sita Ramam', note: null },
  { name: 'Sivaji', note: null },
  { name: 'Amaran', note: 'the film that gave us Sai Pallavi Intro and Hey Minnale' },
  { name: 'Ghilli', note: null },
  { name: 'Thiruvilaiyaadal Aarambam', note: null },
  { name: 'Velaiilla Pattadhari', note: null },
  { name: 'Singam', note: null },
  { name: 'Singam II', note: null },
  { name: 'Vel', note: null },
  { name: 'Panchathanthiram', note: null },
  { name: 'Her', note: null },
  { name: 'About Time', note: null },
  { name: 'A Man Called Otto', note: null },
  { name: 'Parandhu Po', note: null },
  { name: 'Coolie', note: null },
  { name: 'Vettaiyan', note: null },
  { name: 'Maharaja', note: null },
  { name: 'Maareesan', note: null },
  { name: 'Aan Paavam Pollathathu', note: null },
  { name: 'Dude', note: null },
  { name: 'With Love', note: null },
  { name: 'DNA', note: null },
  { name: 'Nilavuku En Mel Ennadi Kobam', note: null },
  { name: '12th Fail', note: null },
  { name: 'Oh My Kadavule', note: null },
  { name: 'Dragon', note: null },
  { name: 'Love Today', note: null }
];

export const SERIES = [
  { name: 'Made in Heaven', note: null },
  { name: 'Game of Thrones', note: null },
  { name: 'Dexter', note: null },
  { name: 'Kim\'s Convenience', note: null },
  { name: 'Bridgerton', note: 'Colin\'s speech to Penelope, reworked and sent Aug 2024' },
  { name: 'You', note: null },
  { name: 'Baby Reindeer', note: null },
  { name: 'Adolescence', note: null },
  { name: 'Heart Beat', note: null },
  { name: 'Brooklyn Nine-Nine', note: null },
  { name: 'Georgie & Mandy\'s First Marriage', note: null },
  { name: 'The Neighborhood', note: null },
  { name: 'Suzhal – The Vortex', note: null },
  { name: 'Vadhandhi – The Fable of Velonie', note: null },
  { name: 'Kuttram Purindhavan', note: null }
];

export const PLACES = [
  { name: 'Apache Pizza (Blackrock)', note: 'our first Valentine\'s, walked in the freezing cold' },
  { name: 'Boeuf and Frites', note: null },
  { name: 'Zaytoon', note: 'our first official date, Feb 6 2024, freezing even inside' },
  { name: 'Daata', note: 'where you fell in love with the pistachio milk cake' },
  { name: 'Bray Beach', note: 'coffee in cups, wind, borrowed car, two hours only' },
  { name: 'Hubert Park', note: 'first picnic, JAI dance, spontaneous summer' },
  { name: 'Blackrock Beach', note: 'the drunk night the first I love you was said' },
  { name: 'Frascati Shopping Centre', note: 'where we hid from the storm on the first failed movie day' },
  { name: 'Penneys', note: 'first shopping together, ended up going at 4 instead of 2' },
  { name: 'Sandyford (OSLO)', note: 'the viewing I went to before we made it to Diwali brunch' },
  { name: 'Rathfarnham', note: 'the January 4 job hunt loop' },
  { name: 'City Centre', note: 'the January 4 job hunt loop, part two' },
  { name: 'Dundrum', note: 'the January 4 job hunt loop, part three' },
  { name: 'Aldi', note: 'the rented-cycle grocery ritual, from January 25 2024 onwards' },
  { name: 'Proby House', note: 'my UCD room. Where it began before it was official.' },
  { name: '26 Woodbine', note: 'our first lease. March 6 to October 31, 2024.' },
  { name: '18 Potters Hill', note: 'the longest one. Nov 1 2024 to Dec 4 2025. 399 nights.' }
];

export const TRIPS = [
  {
    name: 'Edinburgh',
    img: 'edinburgh.jpg',
    caption: 'the first flight together',
    dates: 'April 5 — April 8, 2025',
    body: 'Our first international trip together. Landed on the 8th. Everything felt new, and old, and ours.',
    special: false
  },
  {
    name: 'Wexford & Waterford',
    img: 'waterford-wexford.jpg',
    caption: 'kayaking, and the drive back',
    dates: 'September 19 — September 20, 2025',
    body: 'Second anniversary trip. Kayaking with sea caves that we approached like professionals for our first time. Long drive in the car together. It was so much fun.',
    special: false
  },
  {
    name: 'Cork',
    img: 'cork.jpg',
    caption: 'Namak, and the honesty',
    dates: 'July 20 — July 21, 2026',
    body: 'Right after the FIFA World Cup finals. The last road trip inside Ireland. We talked about how it was going to be the last. The first time we went drinking alone together. It was not romantic like the others, but it was special because it was honest.',
    special: false
  },
  {
    name: 'Iceland',
    img: 'iceland.jpg',
    caption: 'the final one',
    dates: 'September 15 — September 20, 2026',
    body: 'The final one. Your birthday on the 15th. Our anniversary on the 19th. This book was hand-delivered to you here.',
    special: true
  },
  {
    name: 'Milan & Venice',
    img: null,
    caption: 'After. Because we still wanted to.',
    dates: 'October 9 — October 12, 2026',
    body: 'After. Because we still wanted to.',
    special: false
  }
];

export const DICTIONARY = [
  {
    shelf: 'What Aadi calls Krithika',
    entries: [
      { term: 'Ammu', def: 'the first name, the most-used name' },
      { term: 'Chellam', def: 'sweetheart, the classic' },
      { term: 'Chellam-a', def: 'chellam with the softening -a' },
      { term: 'Kannamma', def: 'my eyes, my everything' },
      { term: 'Papu', def: 'baby, the soft one' },
      { term: 'Mu', def: 'shorthand for Ammu when I am typing fast' },
      { term: 'Thangam', def: 'gold' },
      { term: 'Thangamma', def: 'gold-a' },
      { term: 'Chinnu', def: 'little one' },
      { term: 'Girl', def: 'when it needs to be plain' }
    ]
  },
  {
    shelf: 'What Krithika calls Aadi',
    entries: [
      { term: 'Porikki', def: 'said 212 times. rascal, mischief-maker, love word.' },
      { term: 'Ammu', def: 'we share the name, the dragons handle the difference' },
      { term: 'Good boi', def: 'from Sept 29 2023 onwards' },
      { term: 'Passenger princess', def: 'from Sept 28 2023 onwards' },
      { term: 'Chellam', def: null },
      { term: 'Puppy', def: null },
      { term: 'Da', def: 'when the tone is more serious' },
      { term: 'Aadi', def: 'when the tone is even more serious' }
    ]
  },
  {
    shelf: 'What we call each other',
    entries: [
      { term: 'Miss you love you', def: 'the ritual goodnight, one word by now' },
      { term: 'Chellam', def: 'both ways, freely' },
      { term: 'Ammu', def: 'both are Ammu' }
    ]
  }
];

export const FIRSTS = [
  { date: '13 Feb 2023', body: 'First Facebook message. The opener was so bad. She replied anyway.' },
  { date: '2 Jun 2023', body: 'First coffee in Chennai. Months before Ireland ever entered the picture.' },
  { date: '1 Sep 2023', body: 'First meet in Dublin, at the airport.' },
  { date: '15 Sep 2023', body: 'First photo taken together.' },
  { date: '18 Sep 2023', body: 'First kiss.' },
  { date: '19 Sep 2023', body: 'Anniversary. First kiss is a decision, the day after is a promise.' },
  { date: '23 Sep 2023', body: 'First party and drinking together.' },
  { date: '27 Sep 2023', body: 'First sex.' },
  { date: '27 Sep 2023', body: 'First failed job fair. We came back to Proby room because of the wind and rain.' },
  { date: '11 Oct 2023', body: 'First time Aadi got Krithika flowers.' },
  { date: '18 Oct 2023', body: 'The drunk night at Blackrock Beach.' },
  { date: '19 Oct 2023', body: 'Aadi\'s first I love you. Exactly one month after the kiss.' },
  { date: '20 Oct 2023', body: 'First failed movie experience. Leo. Wind so strong the walk from Proby to Frascati took 45 minutes.' },
  { date: '24 Oct 2023', body: 'Successful Leo attempt, 40 minutes late. Never been late to a movie before, but it felt right with the right person.' },
  { date: '26 Oct 2023', body: 'First time Krithika plucked Aadi\'s eyebrows. Became the pre-big-day ritual.' },
  { date: '29 Oct 2023', body: 'First shopping together. Penneys. Was supposed to be 2pm, went at 4pm.' },
  { date: '3 Nov 2023', body: 'First unofficial date. Sat at the beach after. Felt earth was at my feet and I was flying.' },
  { date: '12 Nov 2023', body: 'First Diwali together. OSLO viewing in the morning, lehenga afterwards, cab at noon.' },
  { date: '15 Nov 2023', body: 'First Diwali with friends. Olive green top on the ground, first time quick and silent.' },
  { date: '9 Dec 2023', body: 'Krithika\'s first I love you.' },
  { date: '25 Dec 2023', body: 'First Christmas together.' },
  { date: '27 Dec 2023', body: 'First time Krithika got Aadi flowers. He was bed-ridden sick.' },
  { date: '1 Jan 2024', body: 'First new year together. She wrote "i love the aadi that..." — the litany.' },
  { date: '4 Jan 2024', body: 'First joint job hunt. Rathfarnham, city centre, Dundrum. Left no place unturned.' },
  { date: '18 Jan 2024', body: 'Krithika\'s first part-time job at Eddie Rocket\'s.' },
  { date: '23 Jan 2024', body: 'Aadi\'s first part-time at Applegreen Bakewell Deli.' },
  { date: '25 Jan 2024', body: 'First rented-cycle grocery run to Aldi. Became the ritual.' },
  { date: '6 Feb 2024', body: 'First official date, at Zaytoon. Five months in. Freezing inside the restaurant.' },
  { date: '14 Feb 2024', body: 'First Valentine\'s at Apache. Walked all the way there in the cold.' },
  { date: '14 Feb 2024', body: 'Aadi asked Krithika to move in.' },
  { date: '16 Feb 2024', body: 'First house viewing together. Met Sanju and Dhiv. Love won.' },
  { date: '1 Mar 2024', body: 'First and only snow shower together. Magical. Folded her laundry after work while she was still out.' },
  { date: '6 Mar 2024', body: 'Moved in to 26 Woodbine.' },
  { date: '9 Mar 2024', body: 'Aadi flew to Chennai for the first trip back.' },
  { date: '10 Mar 2024', body: 'First "I miss you." First night apart.' },
  { date: '14 Mar 2024', body: 'First sneaking out and getting a room together, in Chennai. Shady OYO near Escape Avenue.' },
  { date: '19 Mar 2024', body: 'First sneaking in to her house when her parents were away.' },
  { date: '22 Mar 2024', body: 'First time she bought a phone herself, together.' },
  { date: '26 Mar 2024', body: 'First small trip. OMR resort. Full body massage the day before Aadi\'s birthday, apart.' },
  { date: '11 May 2024', body: 'First gadget bought together. The Chromecast and the projector.' },
  { date: '14 May 2024', body: 'First car rental and Bray Beach experience. With Dhiv and Sanju. Coffee in cups.' },
  { date: '5 Jun 2024', body: 'First picnic together at Hubert Park. The JAI dance.' },
  { date: '10 Jun 2024', body: 'First swim together. Jacuzzi, sauna, steam room. Couple therapy.' },
  { date: '1 Nov 2024', body: 'Moved in to 18 Potters Hill.' },
  { date: '5 Nov 2024', body: 'Krithika\'s AON interview.' },
  { date: '4 Dec 2024', body: 'Graduation day. Walked the stage together. Parents met each other.' },
  { date: '9 Dec 2024', body: 'Krithika\'s start date at AON.' },
  { date: '27 Dec 2024', body: 'First ice skating together.' },
  { date: '1 Jan 2025', body: 'Job treat at Daata. Pistachio milk cake era begins.' },
  { date: '14 Jan 2025', body: 'First Pongal together.' },
  { date: '28 Mar 2025', body: 'First birthday of Aadi\'s spent together.' },
  { date: '5 Apr 2025', body: 'First international trip. Edinburgh. Landed Apr 8.' },
  { date: '11 Jun 2025', body: 'First time Krithika picked Aadi up at the airport, after Chennai.' },
  { date: '12 Jul 2025', body: 'First proper dip at a beach together in Dublin. Walked in deep.' },
  { date: '25 Jul 2025', body: 'First theatre experience. Interactive jury duty piece.' },
  { date: '7 Sep 2025', body: 'First time Aadi picked Krithika up at the airport after London. The bouquet ritual begins.' },
  { date: '13 Sep 2025', body: 'First night dogsitting together.' },
  { date: '19 Sep 2025', body: 'Second anniversary. Wexford + Waterford. First kayaking. Sea caves.' },
  { date: '16 Nov 2025', body: 'First properly planned and enjoyed Christmas dinner. Roast, barotta, Baileys, salna.' },
  { date: '5 Dec 2025', body: 'Moved in to the third and final house.' },
  { date: '25 Jan 2026', body: 'Pre-planned valentine day, because we would be apart for the real one.' },
  { date: '5 Apr 2026', body: 'First pottery painting together. Thai massage after. Michelin recommended restaurant. Full day.' },
  { date: '8 Aug 2026', body: 'Both got iPhone 17 Pro. Exchanged SIM slots even though the phones are the same colour.' }
];

export const LASTS = [
  { date: '16 Nov 2025', body: 'Last time on the DJ floor together, pre-booze. Did not know it would be the last.' },
  { date: '25 Jan 2026', body: 'Last Valentine\'s day, celebrated 20 days early because we would be apart on the day.' },
  { date: '20 Apr 2026', body: 'Last Tamil event together. Did not know it would be the last.' },
  { date: '24 May 2026', body: 'Last picnic together. Second one for us.' },
  { date: '20 Jul 2026', body: 'Last road trip in Ireland. Cork. First and last time drinking alone together.' },
  { date: 'Oct 2025', body: 'The month I noticed we were not taking as many pictures as we used to. And we were arriving to places on time.' },
  { date: '4 Dec 2025', body: 'Last night at 18 Potters Hill. 399 nights had lived there.' }
];

// Numbers band at top of Ledger page
export const NUMBERS = [
  { value: '50', label: 'days between message one and first I love you' },
  { value: '3 mo', label: 'of Chennai before Ireland ever existed' },
  { value: '283', label: 'I love yous. 189 mine, 94 hers.' },
  { value: '250', label: 'porikkis. 212 hers, 38 mine.' },
  { value: '204', label: 'miss yous' },
  { value: '4', label: 'homes' },
  { value: '399', label: 'nights at Potters Hill' },
  { value: 'D04 E489', label: 'the first address that was ours' },
  { value: 'Sept 19', label: 'every year, always' }
];

// Care-acts. To be populated via the Ledger interview.
export const LEDGER = [
  { who: 'Aadi', date: 'Feb 14 2024', what: 'Booked Apache for our first Valentine\'s. Walked all the way there in the cold because you wanted the surprise.' },
  { who: 'Krithika', date: 'Dec 27 2023', what: 'Got me flowers when I was bed-ridden sick. Was one of the reasons the sick lifted.' },
  { who: 'Aadi', date: 'Mar 1 2024', what: 'Walked to your room after work in the snow day, folded your laundry and brought it home so you did not have to.' },
  { who: 'Krithika', date: 'Oct 26 2023 onwards', what: 'Plucked my eyebrows before every big day. Became the ritual before every outing.' },
  { who: 'Both', date: 'Sept 7 2025 onwards', what: 'The airport bouquet. Every landing, both directions. Never missed one.' }
];

// On This Day: date → memory. Date format MM-DD for matching regardless of year.
export const ON_THIS_DAY = [
  { date: '02-13', year: 2023, body: 'The day I first messaged you on Facebook. The opener was so bad. You replied anyway.' },
  { date: '06-02', year: 2023, body: 'Our first coffee in Chennai.' },
  { date: '09-01', year: 2023, body: 'First meet in Dublin, at the airport.' },
  { date: '09-15', year: 2023, body: 'The first photo taken of us.' },
  { date: '09-18', year: 2023, body: 'First kiss.' },
  { date: '09-19', year: 2023, body: 'Our anniversary. Every year, always.' },
  { date: '09-23', year: 2023, body: 'First party. First drinking together.' },
  { date: '09-27', year: 2023, body: 'First sex. First failed job fair. Wind and rain brought us home.' },
  { date: '10-11', year: 2023, body: 'First time I got you flowers.' },
  { date: '10-14', year: 2023, body: 'The day you first called me porikki. The word count is at 250 now.' },
  { date: '10-18', year: 2023, body: 'The drunk night at Blackrock Beach.' },
  { date: '10-19', year: 2023, body: 'The day I said I love you first. One month exactly after the kiss.' },
  { date: '10-20', year: 2023, body: '"You are the fuel which makes me keep going." Said out of nowhere, because it was true.' },
  { date: '10-24', year: 2023, body: 'Leo. 40 minutes late. It felt right.' },
  { date: '10-26', year: 2023, body: 'You plucked my eyebrows for the first time. This is now a ritual.' },
  { date: '11-03', year: 2023, body: 'Our first unofficial date. The beach after. Earth at my feet, flying.' },
  { date: '11-12', year: 2023, body: 'First Diwali together. Lehenga. Cab at noon because we could not keep our hands off each other.' },
  { date: '12-09', year: 2023, body: 'The night you first said I love you back.' },
  { date: '12-25', year: 2023, body: 'First Christmas together.' },
  { date: '12-27', year: 2023, body: 'You brought me flowers when I was sick.' },
  { date: '01-01', year: 2024, body: 'You wrote the "i love the aadi that..." litany.' },
  { date: '01-04', year: 2024, body: 'First joint job hunt. Rathfarnham, city centre, Dundrum.' },
  { date: '01-25', year: 2024, body: 'First rented-cycle grocery run. Ritual begins.' },
  { date: '02-06', year: 2024, body: 'First official date, at Zaytoon.' },
  { date: '02-14', year: 2024, body: 'First Valentine\'s, at Apache. Walked in the cold.' },
  { date: '03-01', year: 2024, body: 'First and only snow shower together.' },
  { date: '03-06', year: 2024, body: 'Moved in to 26 Woodbine.' },
  { date: '03-10', year: 2024, body: 'The first "I miss you." First night apart.' },
  { date: '03-13', year: 2024, body: '"Thank you for existing."' },
  { date: '03-28', year: 2024, body: 'My birthday. We could not spend it together this year.' },
  { date: '05-11', year: 2024, body: 'First gadget bought together. Chromecast and projector.' },
  { date: '06-05', year: 2024, body: 'First picnic. Hubert Park. The JAI dance.' },
  { date: '06-10', year: 2024, body: 'First swim. Sauna. Jacuzzi.' },
  { date: '09-15', year: 2024, body: 'Your birthday.' },
  { date: '11-01', year: 2024, body: 'Moved in to 18 Potters Hill.' },
  { date: '12-04', year: 2024, body: 'Graduation. We walked the stage together. Our parents met.' },
  { date: '12-19', year: 2024, body: '"There can\'t possibly be a person out in the world for me, except you."' },
  { date: '12-27', year: 2024, body: 'First ice skating.' },
  { date: '01-01', year: 2025, body: 'Daata. Pistachio milk cake.' },
  { date: '01-14', year: 2025, body: 'First Pongal together.' },
  { date: '04-05', year: 2025, body: 'First international trip. Edinburgh.' },
  { date: '05-15', year: 2025, body: 'The first fight that led to the first breakup talk. Chennai.' },
  { date: '07-12', year: 2025, body: 'First proper beach dip together in Dublin.' },
  { date: '07-25', year: 2025, body: 'First theatre. Interactive jury duty piece.' },
  { date: '09-07', year: 2025, body: 'First airport pickup with bouquet. Ritual begins.' },
  { date: '09-19', year: 2025, body: 'Second anniversary trip. Wexford + Waterford. Kayaking.' },
  { date: '11-16', year: 2025, body: 'The last night on the DJ floor. First proper Christmas dinner planned.' },
  { date: '12-05', year: 2025, body: 'Moved in to the third and final house.' },
  { date: '01-25', year: 2026, body: 'Pre-planned Valentine\'s celebration.' },
  { date: '02-14', year: 2026, body: 'Our first Valentine\'s spent apart. "This life without you is boring and unlivable."' },
  { date: '02-23', year: 2026, body: 'The rule. If it became long distance, we would break up.' },
  { date: '04-05', year: 2026, body: 'Pottery painting day. Thai massage. Michelin restaurant.' },
  { date: '04-20', year: 2026, body: 'Last Tamil event together.' },
  { date: '05-24', year: 2026, body: 'Last picnic.' },
  { date: '07-20', year: 2026, body: 'Last road trip. Cork. First drinking alone together.' },
  { date: '08-08', year: 2026, body: 'iPhone 17 Pro day. Swapped SIM slots.' }
];

// Line and Reply exchanges. Each item is one turn Aadi selects: the setup line, then the reveal.
export const LINE_AND_REPLY = [
  {
    date: 'December 2023',
    line: { speaker: 'Krithika', text: `enna crop pantu dp vechirkeeya da porikki` },
    reply: { speaker: 'Aadi', text: `Indha pic la nee ilaye da ammu` }
  },
  {
    date: 'March 2024',
    line: { speaker: 'Krithika', text: `nan endhrichadhe 4 maniku so no brekkie no lunch\nonly dinner😂😂😂\nlike i said, verychill day` },
    reply: { speaker: 'Aadi', text: `I am hereeee\n😂😂how cute can you beee\nI knew! Wanted to ask if that was your first meal of the day, but I was certain that it was\nOne phone of yours in my camera roll!! One more reason to be excited to get your pic🥰\n🥰🥰everything fair in love and war🥰🥰` }
  },
  {
    date: 'March 2024',
    line: { speaker: 'Aadi', text: `Ammu good night da\nKandipa thungita… I am hanging up\nI love you di` },
    reply: { speaker: 'Krithika', text: `i hope nee thungita now\nenala thunga mudla, i wasn't asleep but sonna you'd stay up more nala i didn't reply to this message until now\ni can't sleep without you\ni love you too baby` }
  },
  {
    date: 'March 2024',
    line: { speaker: 'Aadi', text: `Appa va hospital la drop panitu naan derm kitta vandhurken\nKovil poitu vandhom` },
    reply: { speaker: 'Krithika', text: `🥺🥺🥺 chellam\nyou're such a sweetheart\nit was worth it... i've seen you evolve so much in such a short span of time and it's been unbelievable\ni love that I've never had to bring up the same issue more than twice with you, because you always listen when i say i don't like something that you do and you actually DO something about it\nand i'm glad i did` }
  },
  {
    date: 'October 2023, weeks before the anniversary',
    line: { speaker: 'Aadi', text: `You know, you have got a life you have been yearning for all these years. The type of girlfriend you have been longing for, the freedom you wanted, the kindness of the people around you, the constant verbal affirmations from others and the sense of people actually missing your presence!\nThe girlfriend you have always wanted. The girl of your dreams, just right beside you, day in, day out there with you. All that runs in your mind is not how you gonna conquer the world but just to be with her. The passion, the energy, the fulfilment, and the happiness I you get is unimaginable to say the least.\nHave you ever wanted to see the end of things? Like you wanna know how it's gonna look like in the future, you really wanna know if the ending is gonna be a happy one or not?\nI am feeling it right now with you, I wanna know how things are gonna go and it's just getting me hyped up every single day. hyped up to an extent where I really wanna zoom past time to see where are go. The saying, "சாகும் நாள் தெரிஞ்சா, வாழுகிற நாள் நரகம் ஆகிடும்" is the only things which keeps me going.\nI really am excited, as you say, like a kid.\nEvery single morning, waking up to you, makes things feel easy, days glide with ease, the winter seems bearable, the course feels doable and life out of all feels so colourful and full of joy. I hope, I really hope, this amazing time continues as much as I could see.` },
    reply: { speaker: 'Krithika', text: `the more i read this message, the more it makes my heart smile. you are literally the guy that all those boyfriend reels on instagram are about. i truly lucked out with you, and as my friends said earlier, you definitely are my BIGGEST upgrade.\nthe one aspect of my life that somehow never went right for years together no matter how hard i tried is now effortlessly flowing towards me. and you are the reason. your arms are full of warmth and your eyes are so full of love. and to think that they're all for me makes me smile in a way i can't explain (that's probably the smile that you describe as vekkam). thank you for coming into my life and adding a hint of laughter to everything mundane by simply being yourself.\nthe answer to your question from earlier is yes. you are inarguably the best thing that has happened to me in 2023. and i hope you keep happening to me for as long as possible <3` }
  },
  {
    date: 'August 2024',
    line: { speaker: 'Aadi', text: `And I will run the house from now on. I absolutely understand the fact that you have fought a lot and reached here, and I am not going to snatch that away or burden you with what I should be doing rightfully. You are right when you said everything yesterday. I am sorry for being an asshole.\nYou will not feel like you are running our life and I am not going to let you down again` },
    reply: { speaker: 'Krithika', text: `🥺🥺🥺 i really hope so\nthank you\ni love you too :(` }
  },
  {
    date: 'November 2024',
    line: { speaker: 'Aadi', text: `I love you so much baby\nI feel like I was dragging myself from our house.:(` },
    reply: { speaker: 'Krithika', text: `i love you too bb🥺🥺🥺\nwalking into an empty room felt very weirdly painful for the first time in a long time\ni know you're going to be back in no time\nbut\ni miss you already` }
  },
  {
    date: 'November 2024, right after the worst of it',
    line: { speaker: 'Aadi', text: `I dont know if you still believe in us after what we went through and we were at a stage where amma appa vantu ponadhuku aprom we will split nu and out of nowhere last week was so good (enaku mattum dhaan apdiyaa ila unakum ah nu theril, lmk)\nBut after spending the day away from you and missing you… I realised I love you more than I have ever acknowledged and the way you treat me, nobody ever could. The love care and joy you bring to me is immense and I love that to be with me for the rest of my life. Unaku innum ennoda iruka pudikala mudila na i'd get it but idhu enaku solanum nu thonuchu ammu\nI love you and the place I have for you in my heart is huge.` },
    reply: { speaker: 'Krithika', text: `🥺🥺🥺🥺 the past week has been really good for me too thangam\nidk if i can get ahead of myself and say i have hope for/believe in us cuz it's genuinely too soon to tell\nbut i do really wish it were that way\nbut if our life does continue to be this way, then i guess we'd get there` }
  },
  {
    date: 'April 2025',
    line: { speaker: 'Aadi', text: `Muuu 🐮\nI miss you\nHope I haven't packed once and for all and we continue to be better than ever\nI really felt like I was packing thing up for one last time and that is all the more reason that I am missing you even more\nI love you\nAnd we are going to make it\nWe will communicate and do things necessary\nSee you in chennai\nTalk to you from chennai\nByee` },
    reply: { speaker: 'Krithika', text: `🥺🥺🥺🥺\ni hope so too ammu\ni love you :(\nvellakaran ah pa neenu\nnamma aalungalaye paathu weird aguma unaku` }
  },
  {
    date: 'January 2025',
    line: { speaker: 'Aadi', text: `Reached thangam\nLove you so much\nIt was so good seeing you in the morning as soon as I woke up\nYou have no idea how much it meant to me\nEven though i guessed this would happen\nI love you so much baby` },
    reply: { speaker: 'Krithika', text: `awww\ncutu\ni love you too🥰🥰🥰\nveetuku vantu wake me up lol` }
  },
  {
    date: 'October 2025',
    line: { speaker: 'Aadi', text: `To my cute lil gangster who's facing the pain monster, I know your body's really not being kind to you rn. You don't have to force yourself to do anything da ammu… Just rest, breathe, and take things minute by minute. I'll handle whatever I can for you from wherever I can. You don't have to do this all alone kannama\nAnd I know you feel completely drained and sick rn baby… just let yourself rest… rest as much as you can mu🥺🥺` },
    reply: { speaker: 'Krithika', text: `🥺🥺🥺🥺 cutu ammu\nfirst line siripu vanchu` }
  },
  {
    date: 'January 2026',
    line: { speaker: 'Aadi', text: `Yess ammuu\n:(((\nGn pa\nLove you baby…🥺\nI miss sleeping with you\nIndha vaati night night call kuda panna mudila` },
    reply: { speaker: 'Krithika', text: `i love you too ammu🥺 i can't wait to see u again\nyes cha semmmaaa busy ah podhu\nbut issok we back to livint together in two weeks exactly 😭\nor maybe less actually\n10 days\nok tata baaii gn sd🥰 sleep well and get well soon thangam` }
  },
  {
    date: 'August 2026',
    line: { speaker: 'Aadi', text: `Gnnn… i love you so much… know that you made my day🥺\nYou are the love of my life❤️❤️\nThanks da\nGood night love you miss you kiss you hug you❤️❤️🥰🥺` },
    reply: { speaker: 'Krithika', text: `🥺🥺🥺🥺🥺\ni love you ammu\ni'm glad ammu. i hope you had a good day yourself and that it feels good to be active again 🥰\ngunight tata seeu soon missu kissu` }
  },
  {
    date: 'August 2026, a few weeks ago',
    line: { speaker: 'Krithika', text: `⭐️⭐️⭐️⭐️⭐️\nVisited Chef Aadi's at-home restaurant just yesterday, and I have absolutely no complaints. The day's menu was so carefully curated to suit the visitor's taste and the experience was truly one of a kind. You could really see the thought and effort that went into making every single dish. The taste was out of the world, with a variety of options to choose from. From toast and soup, cookies, and chocolate lava cake to pancakes, pesto pasta, and chicken bites, they had it all. The well-planned menu, timely service, and incredibly tasty food makes this place a must visit. For anyone in/around Leopardstown, I would 10/10 recommend trying this place out - it will not disappoint!\nammu ku review` },
    reply: { speaker: 'Aadi', text: `Ammu… I don't even know if I have the right words to tell you how much this message means to me. idha padicha aprom  manasuku evlo aarudhalaa iruku theriyuma, it honestly lifted such a massive weight off my chest. To know that I made you feel safe enough to be 'one of the girls' is the highest honor I could ever ask for. You were never just my partner, my love, you have been my absolute best friend. Thank you for seeing the good in me, for being so incredibly kind, and for overlooking flaws as much as you could. I am so lucky to have been your golden retriever. I promise you with everything have that we are going to make the absolute most of these next couple of months. we aree going to live out those dreams and make memories that we will both smile about for the rest of our lives...\nhehehe thanksss ammuu means a lott\nlove you di` }
  },
  {
    date: "New Year's Day, 2024",
    line: { speaker: 'Krithika', text: `i love the aadi that acts like a kid that works towards winning my approval, i love the aadi that's a real rough man in bed, i love the aadi that looks into my eyes and sings and writes for and about me like he means all of it, and i love the aadi that always apologises when he's wrong no matter how long it takes and tries everything in his capacity (be it reassurance, or physical love, or constant apologising, or singing/playing a song asking for forgiveness, or cooking something for me) to make sure he makes it up to me before we go to bed\ni love all of you baby<3` },
    reply: { speaker: 'Aadi', text: `I will love you in every single way I could.` }
  },
  {
    date: 'her birthday, September 2025',
    line: { speaker: 'Krithika', text: `hi ammu,\ni don't think i took the proper time out to actually appreciate you for a wonderful surprise when the clock struck 12 on my birthday. it's something i've wanted and dreamt of for a really long time and you've made that my reality, and i can't thank you enough for it. it was magical when i walked into the room - i literally felt like the main character and the room was to celebrate me. and i love the gifts too - every detail of this was so thoughtful and well-planned. i can't wait to get some colouring pencils and get started on the book hehe i used to love them so much as a kid and it completely slipped my mind to start again until just yesterday. i'd love to do the 1000-piece puzzle together with you if you'd like?\ni know our relationship has a long way to go, and i'm still unsure of what the future holds. but regardless of where we end up, i want you to know that although i felt crappy through this birthday for a long list of reasons, if there was a silver lining, it's this surprise. and i will always be grateful for all the effort that went into putting it together (especially by yourself, when we have no family or friends around to help). it really means a lot to me. thank you ❤️` },
    reply: { speaker: 'Aadi', text: `Hiii ammu ❤️\nI am sooo happy reading this… like…I'm so so happyyyy.\nI really wanted you to feel celebrated the way you truly deserve to, every time. Every single day. And I am really glad that you felt like the main character, and that was the idea.\nYou really are the main character to me di 🥹✨ and I loved puttingall this together for you.\nPlanning, prepping and making ellame… \nCant wait for you to start with the colouring book and show me the things you have done🥰 and yesss, kandipa we can do the puzzle together!!! That will be so much fun 🧩\nI love you... and making you feel special like this will always be one of my favourite things to do. 🥰\nAnd I am really sorry that things didn't go as planned and yoi crappy because of me. I am really sorry:(` }
  }
];

// Section grid on Contents home
export const SECTIONS = [
  { slug: 'foreword', title: 'Foreword', sub: 'a note before you begin', dragon: 'letter' },
  { slug: 'cast', title: 'The Cast', sub: 'two dragons, one story', dragon: 'hug' },
  { slug: 'songs', title: 'Songs', sub: 'eleven songs, one for each part', dragon: 'note' },
  { slug: 'watched', title: 'What We Watched', sub: 'films and series we lived inside', dragon: 'stars' },
  { slug: 'places', title: 'Our Places', sub: 'the map of Dublin, ours', dragon: 'nest' },
  { slug: 'trips', title: 'Our Trips', sub: 'four before, one to come', dragon: 'adventure' },
  { slug: 'dictionary', title: 'The Dictionary', sub: 'words only we use', dragon: 'bubble' },
  { slug: 'firsts', title: 'Firsts & Lasts', sub: 'a timeline of us', dragon: 'clock' },
  { slug: 'ledger', title: 'The Ledger', sub: 'the numbers, and the small acts', dragon: 'heart' },
  { slug: 'onthisday', title: 'On This Day', sub: 'today, one year ago', dragon: 'moon' },
  { slug: 'lineandreply', title: 'The Line and the Reply', sub: 'a conversation, replayed', dragon: 'bubble2' },
  { slug: 'polaroids', title: 'Polaroids', sub: 'moments printed', dragon: 'stamp' },
  { slug: 'voicenotes', title: 'Voice Notes', sub: 'random recordings, hers and mine', dragon: 'mic' },
  { slug: 'timecapsule', title: 'Time Capsule', sub: 'a letter for a year from now', dragon: 'jar' },
  { slug: 'finalsong', title: 'The Final Song', sub: 'the final one', dragon: 'sunset' },
  { slug: 'release', title: 'Release', sub: 'the way out, if you want it', dragon: 'sleeping' }
];
